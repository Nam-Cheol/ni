...
README의 public demo 전체를 source에서 검증한다:

```bash
bash scripts/demo-check.sh
```

demo check는 ambiguous prompt demo의 intentional `BLOCKED` 상태를 assert하고,
locked example prompts를 temporary files로만 컴파일한다. Downstream agents는
실행하지 않는다.
...
변경할 때 실행한다. 자세한 내용은
[docs/48_DEMO_VERIFICATION.md](docs/48_DEMO_VERIFICATION.md)에 있다.
