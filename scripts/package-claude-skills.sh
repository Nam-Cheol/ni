...
DIST_DIR="$ROOT/dist"
OUT="$DIST_DIR/ni-claude-skills.zip"
STAGING_PARENT="${TMPDIR:-/tmp}/ni-claude-skills-package.$$"
STAGING="$STAGING_PARENT/ni-claude-skills"
...
do
  if [[ ! -f "$PACK_ROOT/$required" ]]; then
    echo "missing Claude skill pack file: packages/claude-skills/$required" >&2
    exit 1
  fi
done
