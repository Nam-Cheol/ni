...
| `examples/ni-grill/` | Pre-lock planning challenge UX: deterministic blockers first, then severity-labeled, budgeted `GRILL-*` questions against accepted 또는 nearly accepted content를 묻는다. 또한 isolated `READY` proof 뒤 benchmark grill을 dogfood해 new empirical claims 없이 claim boundaries와 `not_measured` visibility를 테스트한다. | draft `conversation_product`; benchmark evidence review | draft `conversation`, `document`; benchmark workspace는 isolated 상태 유지 | claim하지 않음 | required docs, benchmark grill files, severity-labeled `GRILL-*` findings, lessons, non-execution boundary wording만 확인한다. | 예, transcript fixture plus benchmark review transcripts. | Yes |
...
ni-grill benchmark transcripts도 non-execution example이다. Isolated `ni status`
proof를 기록하고 benchmark claim boundaries를 challenge하지만, generated prompts를
실행하거나, model APIs를 호출하거나, fieldwork를 수행하거나, products를
implement하거나, empirical claims를 만들지 않는다.
