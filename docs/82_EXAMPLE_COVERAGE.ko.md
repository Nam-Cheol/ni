...
| `examples/benchmark-report/` | `not_measured` boundary가 있는 benchmark/case-study reporting method와 measured dashboard 및 research-protocol case를 증명한다. Dashboard case는 blocked-to-ready artifact-readiness transition을 package하고, research-protocol case는 non-software study request의 initial `BLOCKED` readiness를 측정한다. | Dashboard artifact case는 `document_product`; research case는 `research_protocol` | Dashboard는 `document`; research는 `document`, `workflow`, `human_service` | Dashboard case: benchmark artifact readiness에 한해 `READY`; research case: initial readiness `BLOCKED` | required docs, dashboard historical blocked/resolved proof, research initial blocked proof, blocker/next-question evidence, lock/prompt summary가 있는 경우, product/runtime/research claim의 남은 `not_measured` boundary를 확인한다. | 부분적: report template은 docs-only이고 dashboard case는 locked ni workspace를 가진다. research case는 lock이 없는 intentionally blocked workspace다. | Yes |
