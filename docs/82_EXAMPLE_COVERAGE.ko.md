...
Benchmark example에서는
[`97_BENCHMARK_CLAIM_BOUNDARIES.ko.md`](97_BENCHMARK_CLAIM_BOUNDARIES.ko.md)의
claim-boundary label을 모든 `READY` transition과 `not_measured` table 옆에 둔다.
...
| `examples/ni-grill/` | Pre-lock planning challenge UX: deterministic blockers first, then severity-labeled, budgeted `GRILL-*` questions against accepted 또는 nearly accepted content를 묻는다. 또한 benchmark grill과 ni 자체 current planning state를 dogfood해 new empirical claims 없이 claim boundaries, roadmap specificity, `not_measured` visibility를 테스트한다. | draft `conversation_product`; benchmark evidence review; ni planning review | draft `conversation`, `document`; benchmark workspace는 isolated 상태 유지 | claim하지 않음 | required docs, benchmark grill files, ni-project dogfood file, severity-labeled `GRILL-*` findings, lessons, non-execution boundary wording만 확인한다. | 예, transcript fixture plus benchmark and ni-project review transcripts. | Yes |
| `examples/benchmark-report/` | `not_measured` boundary가 있는 benchmark/case-study reporting method와 measured dashboard 및 research-protocol case를 증명한다. Dashboard case는 blocked-to-ready artifact-readiness transition을 package한다. Research-protocol case는 initial `BLOCKED` readiness를 보존한 뒤 synthetic fixture answer를 적용해 isolated-workspace `READY`, lock, bounded prompt proof에 도달한다. Claim boundary는 `docs/97_BENCHMARK_CLAIM_BOUNDARIES.ko.md`를 따른다. | Dashboard artifact case는 `document_product`; research case는 `research_protocol` | Dashboard는 `document`; research는 `document`, `workflow`, `human_service` | Dashboard case: benchmark artifact readiness에 한해 `READY`; research case: synthetic benchmark fixture readiness에 한해 `READY` | required docs, dashboard historical blocked/resolved proof, research initial blocked proof, research resolved proof, blocker/next-question evidence, blocker analysis, resolution path, answer packet, lock/prompt summary, before/after evidence, lessons, product/runtime/research claim의 남은 `not_measured` boundary를 확인한다. Shared claim-boundary marker document도 확인한다. | 부분적: report template은 docs-only이고 dashboard case는 locked ni workspace를 가진다. research case도 synthetic fixture answer 이후 locked isolated benchmark workspace를 가진다. | Yes |
...
ni-grill benchmark transcripts와 ni-project dogfood transcript도 non-execution
example이다. `ni status` proof를 기록하고 planning 또는 benchmark claim
boundaries를 challenge하지만, generated prompts를 실행하거나, model APIs를
호출하거나, fieldwork를 수행하거나, products를 implement하거나, release를
publish하거나, lockfiles를 edit하거나, empirical claims를 만들지 않는다.
