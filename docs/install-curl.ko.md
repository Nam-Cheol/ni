...
Status: verified v0.3.0 GitHub Release assets에 대해 Available이다. `install.sh`는
선택된 archive와 `ni_0.3.0_checksums.txt`를 download하고, local sha256 tool이
...
[v0.3.0 Curl Installer Verification](71_CURL_INSTALLER_VERIFICATION_v0.3.0.ko.md)이
검증한 release를 원하면 `VERSION="0.3.0"`으로 고정한다. 설치된 CLI는 help 또는
...
v0.3.0 verification은 2026-05-28에 통과했다. Installer는
`Verified checksum for ni_0.3.0_darwin_arm64.tar.gz`를 출력했고 temporary
`BINDIR`에 binary를 install했으며, installed binary는 `ni version`에서 `0.3.0`을
반환했다.
