...
이 audit는 current-tree rename이 v0.6.0 release preparation으로 진행할 준비가
...
boundaries는 v0.6.0 release preparation으로 진행할 준비가 되어 있다. Public
v0.6.0 install retrieval, hosted artifacts, Windows real-host execution,
Homebrew availability, external user validation은 future evidence gate로 남는다.

## Current-tree rename readiness

| Surface | Expected | Observed | Pass? | Notes |
| --- | --- | --- | --- | --- |
| product name | Namba Intent | README, CLI help, docs가 current-tree surfaces에서 Namba Intent를 사용한다. | Yes | v0.6.0은 published 아님. |
...
| --- | --- | --- | --- | --- |
| README.md | current main과 latest public v0.5.1을 분리한다. | v0.6.0은 upcoming이고 v0.5.1은 여전히 `ni`일 수 있다고 말한다. | Yes | v0.6.0 verification 전까지 note 유지. |
...
| docs/22_INSTALL.md | Public v0.5.1과 current-main v0.6.0 paths를 분리한다. | Public `namba-intent` installer retrieval을 Release-gated로 update했다. | Yes | v0.6.0 publication 뒤 reverify. |
...
| public latest release | Latest public release는 v0.5.1일 수 있다. | `git tag --list v0.5.1`은 `v0.5.1`; `v0.6.0` tag absent. | Yes | Current tree를 published release로 취급하지 않는다. |
| v0.6.0 release status | Not published. | Local `v0.6.0` tag 없음. | Yes | Publication은 future human-approved task. |
...
| checksum names | Future checksum file은 `namba-intent_<version>_checksums.txt`. | Config와 installers가 match한다. | Yes | Public v0.6.0 checksum not published. |
...
| --- | --- | --- | --- |
| v0.6.0 publication | 이 task는 non-publishing. | Human-approved tag, release workflow, release metadata. | No, publication claim만 block. |
| public install retrieval of namba-intent | Hosted v0.6.0 assets가 아직 없다. | Published v0.6.0 assets에서 isolated install + help/version proof. | No, public install claim만 block. |
| hosted artifacts | Release action 없음. | Asset inventory and checksum verification. | No, hosted artifact claim만 block. |
...
| --- | --- | --- | --- | --- |
| v0.6.0 publication status | Must say not published. | Preserved. | Yes | Tag/release action 없음. |
| public install | Public `namba-intent` retrieval claim 금지. | docs/22와 curl docs가 Release-gated로 표시한다. | Yes | v0.5.1 evidence는 `ni`. |
| Namba Intent identity | Current tree may use it. | README/help/docs에 사용. | Yes | Release claim은 future-gated. |
| namba-intent command | Primary current-tree command. | Implemented and validated. | Yes | Public install not verified. |
