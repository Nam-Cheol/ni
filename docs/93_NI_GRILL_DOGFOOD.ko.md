...
| GRILL-005 | Note | model workspace status | `docs/55_MODEL_WORKSPACE_PACKS.md`; `docs/75_MODEL_PACK_INSTALL_VERIFICATION.md`; `docs/99_MODEL_WORKSPACE_STATUS.ko.md`; `packages/*-skills/**` | `docs/99_MODEL_WORKSPACE_STATUS.ko.md`로 addressed; host verification은 future work | no |
...
5. GRILL-005 — Note — model workspace status
   ...
   Concern: Model workspace pack statuses는 현재 factual하다. Repo-local source files, package roots, metadata checks, zip packaging은 verified repository evidence이고, broad model workspace status는 Experimental이며, global host discovery는 unverified이고, no-terminal은 assisted이고, skills는 반복해서 "Skills are UX; CLI is authority."라고 말한다.
   Why it matters: 이 wording은 model packs가 CLI authority나 execution adapters로 오해되는 일을 막는다.
   Question: Future model-pack edits에서도 현재 status split, 즉 verified repository evidence, Experimental broad product path, not_verified global host discovery/provider behavior, CLI authority를 유지할까?
...
`docs/97_BENCHMARK_CLAIM_BOUNDARIES.ko.md`로 addressed되었다. GRILL-005는
status preservation boundary로 `docs/99_MODEL_WORKSPACE_STATUS.ko.md`에서
addressed되었지만, actual host-level install과 provider behavior verification은
future work이며 model workspace packs는 broad product path로 Experimental이다.
