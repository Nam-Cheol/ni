...
- Skills는 amended planning text를 draft하고 `LOCK-STALE`를 explain할 수 있지만
  readiness를 determine하거나, lock/relock하거나, `ni status`, `ni end`,
  `ni run`을 replace하거나, `.ni/plan.lock.json`을 update하지 않는다.
- `LOCK-STALE`가 나타나면 다음 recovery order를 보존한다:
