...
2026-05-28 기준으로 `https://github.com/Nam-Cheol/homebrew-tap.git`에 대한
...
Tap repository가 아직 존재하지 않으므로 Homebrew status는 Planned로 남긴다.
Owner가 tap을 만들 준비가 되었을 때 다음 exact next steps를 사용한다:
...
4. [Homebrew Formula Draft](71_HOMEBREW_FORMULA_DRAFT.md)를 starting point로
   사용하되, 모든 placeholder checksum을 published GitHub Release의 checksum으로
   교체한다.
5. Tap checkout에서 formula를 validate한다:
...
6. Validation이 성공한 뒤에만 tested install command를 다음처럼 문서화한다:

   ```bash
   brew install Nam-Cheol/tap/ni
   ```

그 command가 실제로 동작하기 전까지 Homebrew badge, README install command,
package-manager availability claim을 추가하지 않는다.
