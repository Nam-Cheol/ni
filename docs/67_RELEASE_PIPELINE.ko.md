...
- Release binary availability: tagged GitHub Release가 완료되고 uploaded
  assets와 checksums가 검증되기 전까지는 available하지 않다.
- Curl installer availability: published release assets 기준으로 검증되기
  전까지는 available하지 않다.
