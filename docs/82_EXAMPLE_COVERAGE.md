...
| `examples/benchmark-report/` | Benchmark/case-study reporting method with `not_measured` boundaries, plus measured dashboard and research-protocol cases. The dashboard case packages a blocked-to-ready artifact-readiness transition; the research-protocol case measures initial `BLOCKED` readiness for a non-software study request. | Dashboard artifact case: `document_product`; research case: `research_protocol` | Dashboard: `document`; research: `document`, `workflow`, `human_service` | Dashboard case: `READY` for benchmark artifact readiness only; research case: `BLOCKED` initial readiness | Verifies required docs, dashboard historical blocked/resolved proof, research initial blocked proof, blocker/next-question evidence, lock/prompt summaries where present, and remaining `not_measured` boundaries for product/runtime/research claims. | Partial: report template is docs-only; dashboard case has a locked ni workspace; research case is an intentionally blocked workspace with no lock. | Yes |
