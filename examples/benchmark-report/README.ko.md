...
Research-protocol `ni status` 명령은 initial isolated workspace에 대해
`NI Intent Readiness: BLOCKED`를 보고해야 한다. Research workspace에는 lockfile이
없어야 한다.

`rg` 명령은 template과 dashboard case의 `not_measured` marker, checked-in
blocked/resolved status proof, research case의 `not_measured` marker,
blocker/next-question evidence, benchmark protocol의 non-execution 및
